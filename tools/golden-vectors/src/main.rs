...
use sybil_verifier::{
    ...
    BridgeStateSnapshot, ClientActionWitness, DepositAccumulatorWitness, KeyOpAuth, KeyRecord,
...
use sybil_zk::{
    ...
    state_transition_public_input_hash, witness_root,
};
...
#[derive(Serialize)]
struct EpochPublicInputVector {
    start_height: u64,
    end_height: u64,
    start_state_root: String,
    end_state_root: String,
    block_count: u64,
    blocks_commitment: String,
    epoch_da_commitment: String,
    deposit_root: String,
    deposit_count: u64,
    hash: String,
}
...
fn render() -> Result<String, String> {
    ...
    let epoch_public_inputs = EpochTransitionPublicInputs {
        start_height: 10,
        end_height: 12,
    ...
    let vectors = GoldenVectors {
        ...
        epoch_transition_public_inputs: EpochPublicInputVector {
            start_height: epoch_public_inputs.start_height,
            end_height: epoch_public_inputs.end_height,
            start_state_root: hex_bytes(&epoch_public_inputs.start_state_root),
            end_state_root: hex_bytes(&epoch_public_inputs.end_state_root),
            block_count: epoch_public_inputs.block_count,
            blocks_commitment: hex_bytes(&epoch_public_inputs.blocks_commitment),
            epoch_da_commitment: hex_bytes(&epoch_public_inputs.epoch_da_commitment),
            deposit_root: hex_bytes(&epoch_public_inputs.deposit_root),
            deposit_count: epoch_public_inputs.deposit_count,
            hash: hex_bytes(&epoch_transition_public_input_hash(&epoch_public_inputs)),
        },
        escape_claim_public_inputs: EscapeClaimPublicInputVector {
