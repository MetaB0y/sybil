from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset







T = TypeVar("T", bound="BridgeDomainResponse")



@_attrs_define
class BridgeDomainResponse:
    """ 
        Attributes:
            chain_id (int):
            token_address_hex (str):
            vault_address_hex (str):
     """

    chain_id: int
    token_address_hex: str
    vault_address_hex: str
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)





    def to_dict(self) -> dict[str, Any]:
        chain_id = self.chain_id

        token_address_hex = self.token_address_hex

        vault_address_hex = self.vault_address_hex


        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({
            "chain_id": chain_id,
            "token_address_hex": token_address_hex,
            "vault_address_hex": vault_address_hex,
        })

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        chain_id = d.pop("chain_id")

        token_address_hex = d.pop("token_address_hex")

        vault_address_hex = d.pop("vault_address_hex")

        bridge_domain_response = cls(
            chain_id=chain_id,
            token_address_hex=token_address_hex,
            vault_address_hex=vault_address_hex,
        )


        bridge_domain_response.additional_properties = d
        return bridge_domain_response

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
