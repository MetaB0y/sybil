...
export function EventActivity({ marketId }: { marketId: number }) {
  ...
  const groupUnavailable = groupError != null && group == null;
  const historyUnavailable = candles.unavailableCount > 0;
  const empty =
    groupPending || (!group && !groupUnavailable) || outcomes.length === 0
      ? "loading activity…"
      : bars.length === 0
        ? shownCandles.isPending
          ? "loading volume history…"
          : candles.failureCount === 0
  ...
  return (
    ...
    >
      ...
      {groupUnavailable ? (
        ...
        <ActivityDataNotice
          message="volume history incomplete"
          retrying={candles.isRetrying}
          onRetry={() => void candles.retryFailed()}
        />
      ) : empty != null ? (
        <Empty>{empty}</Empty>
      ) : (
        <>
          {candles.failureCount > 0 && (
            <ActivityDataNotice
              message="volume refresh failed · showing saved history"
              retrying={candles.isRetrying}
              onRetry={() => void candles.retryFailed()}
              stale
            />
