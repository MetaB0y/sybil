...
describe("market filter selection accessibility", () => {
  it("announces the selected category", () => {
    const html = renderToStaticMarkup(<CategoryTabs />);
  ...
  it("announces the selected sort and closed-market filter", () => {
    ...
    expect(html).toMatch(
      /role="group" aria-label="market sorting"[^>]*>.*Volume.*New.*Traders.*<\/div><span/,
    );
    expect(html).toMatch(/aria-pressed="true"[^>]*>New<\/button>/);
