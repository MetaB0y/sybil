...
const mkt = (
  o: Partial<DevMarket> & { market_id: number; name: string },
): DevMarket => o;

describe("dev/derive", () => {
  it("priceState classifies cleared / ref only / no clears", () => {
    expect(
      priceState(mkt({ market_id: 1, name: "a", yes_price_nanos: 5e8 })),
    ).toBe("cleared");
    expect(
      priceState(mkt({ market_id: 1, name: "a", reference_price_nanos: 5e8 })),
    ).toBe("ref only");
    ...
    expect(
      priceGap(
        mkt({
          ...
          name: "a",
          yes_price_nanos: 6e8,
          reference_price_nanos: 5e8,
        }),
      ),
    ).toBeCloseTo(0.1);
    expect(
      priceGap(mkt({ market_id: 1, name: "a", yes_price_nanos: 6e8 })),
    ).toBe(0);
  });
  ...
  it("filterMarkets applies search and the cleared state filter", () => {
    ...
    const search = filterMarkets(
      markets,
      { search: "trump", group: "", state: "all" },
      pendingIndex([]),
    );
    expect(search.map((m) => m.market_id)).toEqual([1]);
    const cleared = filterMarkets(
      markets,
      { search: "", group: "", state: "cleared" },
      pendingIndex([]),
    );
  ...
  it("topMarketsByVolume24h sorts by 24h volume desc", () => {
    ...
    expect(topMarketsByVolume24h(markets).map((m) => m.market_id)).toEqual([
      2, 1,
    ]);
  });

  it("latestBlockByMarketRows expands the by_market sidecar", () => {
    const block: DevBlock = {
      height: 5,
      by_market: { "7": { placers: 2, volume_nanos: 30, matched: 1 } },
    };
  ...
  it("buildInsights always reports price coverage", () => {
    const insights = buildInsights({
      markets: [mkt({ market_id: 1, name: "a", yes_price_nanos: 5e8 })],
      blocks: [],
      pendingOrders: [],
    });
    ...
    expect(
      fmtLiquidity(
        mkt({
          market_id: 1,
          name: "a",
          liquidity_avg10_nanos: 2e9,
          liquidity_band_nanos: 5e8,
        }),
      ),
    ).toBe("$2.00 ±$0.50");
  ...
  it("accountAggregates sums canonical portfolio fields for discovered accounts", () => {
    const accounts: DevAccountPortfolio[] = [
      {
        account_id: 0,
        balance_nanos: 1e9,
        total_deposited_nanos: 1e9,
        portfolio_value_nanos: 1e9,
        positions: [],
      },
      {
        account_id: 1,
        balance_nanos: 5e9,
        portfolio_value_nanos: 6e9,
        positions: [{ market_id: 1, outcome: "YES", quantity: 10 }],
      },
      {
        account_id: 2,
        balance_nanos: 3e9,
        portfolio_value_nanos: 5e9,
        pnl_nanos: 2e9,
        positions: [],
      },
    ];
    const agg = accountAggregates(accounts, null);
    expect(agg.mmCashNanos).toBe(9e9);
    expect(agg.mmPortfolioValueNanos).toBe(12e9);
    expect(agg.mmPnlNanos).toBe(2e9);
    expect(agg.activeTradingAccounts.map((a) => a.account_id)).toEqual([
      1, 2, 0,
    ]);
  });

  it("formatOrder formats side, quantity and price for both price branches", () => {
    expect(formatOrder({ side: "BuyYes", quantity: 10, price: 6e8 })).toBe(
      "BuyYes 10 @ $0.600",
    );
    expect(formatOrder({ side: "BuyNo", quantity: 5, price: 0.42 })).toBe(
      "BuyNo 5 @ 42.0%",
    );
  });
});
