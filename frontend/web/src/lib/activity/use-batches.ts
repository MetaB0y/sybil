...
export type UseBatchPageResult = {
  ...
  hasOlder: boolean;
  /** Failed backfill/page reads are surfaced, never rendered as an empty chain. */
  error: Error | null;
  isRetrying: boolean;
  retry: () => void;
};
...
export function useBatchPage({
  ...
  const rows = useMemo(
    () =>
      (complete ? fromStore : (pageQ.data ?? fromStore)).map(deriveBatchRow),
...
async function fetchBlockPage(
  ...
  const { data, error } = await api.GET("/v1/blocks", {
    params: {
      query: {
        limit,
        ...(beforeHeight == null ? {} : { before_height: beforeHeight }),
      },
    },
  });
