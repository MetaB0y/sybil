...
#[tokio::test]
async fn market_price_serves_book_midpoint_mark_when_quiet() {
    let (app, handle) = test_app(true).await;
    ...
    .await;
    let yes_bidder = parse_json(&body)["account_id"].as_u64().unwrap();
    let (_, body) = post_json(
        app.clone(),
        "/v1/accounts",
        json!({ "initial_balance_nanos": balance }),
    ...
    .await;
    let no_bidder = parse_json(&body)["account_id"].as_u64().unwrap();
    let (_, body) = post_json(app.clone(), "/v1/markets", json!({ "name": "Quiet mark" })).await;
    let market_id = parse_json(&body)["market_id"].as_u64().unwrap();

    for (account_id, side) in [(yes_bidder, "BuyYes"), (no_bidder, "BuyNo")] {
        let (status, body) = post_json(
            app.clone(),
            "/v1/orders",
            json!({
    ...
    assert!(
        !block
            .canonical
            .clearing_prices
            .contains_key(&matching_engine::MarketId::new(market_id as u32))
    );

    let (status, body) = get(app.clone(), "/v1/markets/prices").await;
    assert_eq!(status, StatusCode::OK);
    let prices = parse_json(&body);
    assert_eq!(
        prices["prices"][market_id.to_string()]["yes_price_nanos"],
        json!(500_000_000u64)
    );

    let (status, body) = get(app, "/v1/markets").await;
    assert_eq!(status, StatusCode::OK);
    let markets = parse_json(&body);
    let market = markets
        .as_array()
        .unwrap()
        .iter()
        .find(|market| market["market_id"] == json!(market_id))
        .expect("quiet market");
    assert_eq!(market["yes_price_nanos"], json!(500_000_000u64));
}
