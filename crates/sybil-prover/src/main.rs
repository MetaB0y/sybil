...
enum Command {
    ...
    Worker(WorkerArgs),
    /// Follow the live Sybil API and emit mock proof artifacts with the same status interface.
    MockLive(MockLiveArgs),
...
async fn run(cli: Cli) -> Result<(), ProverCliError> {
    match cli.command {
        ...
        Command::MockLive(args) => mock_live(args).await,
...
async fn mock_live(args: MockLiveArgs) -> Result<(), ProverCliError> {
    std::fs::create_dir_all(&args.artifacts_dir).map_err(|source| ProverCliError::CreateDir {
        path: args.artifacts_dir.clone(),
        source,
    })?;
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|source| ProverCliError::Http { source })?;
...
async fn fetch_latest_block(
    ...
    sybil_url: &str,
) -> Result<BlockResponse, ProverCliError> {
    let resp = client
...
fn write_mock_artifact(
    artifacts_dir: &Path,
    block: &BlockResponse,
) -> Result<bool, ProverCliError> {
...
fn prune_mock_artifacts(artifacts_dir: &Path, max_artifacts: usize) -> Result<(), ProverCliError> {
    if max_artifacts == 0 {
        return Ok(());
    }
    ...
    for dir in dirs.into_iter().take(excess) {
        std::fs::remove_dir_all(&dir)
            .map_err(|source| ProverCliError::Write { path: dir, source })?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn renders_prover_metrics_from_artifact_store() {
        let artifacts_dir = temp_path("metrics-artifacts");
        let jobs_dir = temp_path("metrics-jobs");
        let job_id = StateTransitionProofJobId {
            block_height: 9,
            block_hash: [0x11; 32],
            state_root: [0x22; 32],
        ...
        std::fs::create_dir_all(&artifact_dir).unwrap();
        std::fs::create_dir_all(&jobs_dir).unwrap();
        std::fs::write(jobs_dir.join("next.msgpack"), b"job").unwrap();
        let status = WorkerStatusJson {
            version: 1,
            status: "prepared".to_string(),
            job_path: "/tmp/job.msgpack".to_string(),
        ...
        write_json_pretty(&artifact_dir.join("status.json"), &status).unwrap();

        let metrics = render_prover_metrics(&artifacts_dir, Some(&jobs_dir)).unwrap();
        let _ = std::fs::remove_file(artifact_dir.join("status.json"));
        let _ = std::fs::remove_file(jobs_dir.join("next.msgpack"));
        let _ = std::fs::remove_dir(&artifact_dir);
        let _ = std::fs::remove_dir(&artifacts_dir);
        let _ = std::fs::remove_dir(&jobs_dir);

        assert!(metrics.contains("sybil_prover_artifact_store_ready 1"));
        assert!(metrics.contains("sybil_prover_artifact_directories_total 1"));
        assert!(metrics.contains(
            "sybil_prover_artifacts_total{status=\"prepared\",proof_status=\"not_started\"} 1"
        ));
        assert!(metrics.contains("sybil_prover_latest_prepared_height 9"));
        assert!(metrics.contains("sybil_prover_latest_updated_at_seconds 123"));
        assert!(metrics.contains("sybil_prover_jobs_queued 1"));
    }
