...
impl BlockSequencer {
    ...
    /// Rewrite a live market's committed name and metadata.
    ///
    /// The catalog applier owns the checked-in text for native markets, but
    /// `create_market_with_metadata` refuses to re-key an existing market whose
    /// text drifted (`MarketCreationKeyConflict`). This is the edit path: same
    /// market id, same creation key, new prose.
    ///
    /// Both the name and the metadata digest ride `MarketSnapshot` into the
    /// state root, so this moves the root at the block it lands in — it is a
    /// sequenced state transition, not a display-only patch.
    pub fn update_market_content(
        &mut self,
        market_id: MarketId,
        name: String,
        metadata: MarketMetadata,
    ) -> Result<(), SequencerError> {
        let metadata = self.resolved_market_content(market_id, &name, metadata)?;
        self.markets.rename(market_id, name);
        self.lifecycle.set_market_metadata(market_id, metadata);
        Ok(())
    }

    /// `true` when the market already carries exactly this content, so the
    /// caller can skip a durable write that would change nothing.
    pub fn market_content_matches(
        &self,
        market_id: MarketId,
        name: &str,
        metadata: &MarketMetadata,
    ) -> Result<bool, SequencerError> {
        let resolved = self.resolved_market_content(market_id, name, metadata.clone())?;
        let existing_name = self
            .markets
            .get(market_id)
            .map(|market| market.name.as_str())
            .unwrap_or_default();
        Ok(existing_name == name && self.lifecycle.market_metadata(market_id) == Some(&resolved))
    }

    /// Validate an edit and fold in the fields an edit may not touch.
    ///
    /// Identity is not editable: preserving `creation_key` and `created_at_ms`
    /// keeps one creation key pointing at one market for the life of the
    /// deployment, whatever the request body claims. `resolution_config` is
    /// preserved for the same reason — an edit rewrites prose, and must not be
    /// able to swap the template that settles the market out from under open
    /// positions. `committed_metadata_digest` is a recovery-only override for
    /// DA-imported markets whose witness proved a digest without carrying the
    /// fields — once we write real fields it must go, or the snapshot would
    /// keep publishing a digest of the old text.
    fn resolved_market_content(
        &self,
        market_id: MarketId,
        name: &str,
        mut metadata: MarketMetadata,
    ) -> Result<MarketMetadata, SequencerError> {
        if self.markets.get(market_id).is_none() {
            return Err(SequencerError::MarketNotFound { market_id });
        }
        if name.trim().is_empty() {
            return Err(SequencerError::InvalidMarketName);
        }
        let status = self.lifecycle.market_status(market_id);
        if !status.is_tradeable() {
            return Err(SequencerError::MarketNotTradeable {
        ...
        if let Some(key) = metadata.creation_key.as_deref() {
            validate_market_creation_key(key)?;
        }
        let existing = self.lifecycle.market_metadata(market_id);
        metadata.creation_key = existing.and_then(|meta| meta.creation_key.clone());
        metadata.resolution_config = existing.and_then(|meta| meta.resolution_config.clone());
        if let Some(created_at_ms) = existing.map(|meta| meta.created_at_ms) {
            metadata.created_at_ms = created_at_ms;
        }
        metadata.committed_metadata_digest = None;
        Ok(metadata)
    }
