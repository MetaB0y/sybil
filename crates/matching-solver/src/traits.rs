...
impl PriceProjectionResult {
    /// Create an identity result (no changes needed).
    pub fn identity(prices: HashMap<MarketId, Vec<Nanos>>) -> Self {
        Self {
            prices,
            violations_fixed: 0,
            max_adjustment: 0,
...
/// Projects prices onto constraint-feasible space.
///
/// A `PriceProjector` takes raw prices from price discovery and projects them
/// onto the set of prices that satisfy cross-market constraints (marginal
/// consistency from bundle orders).
///
/// # Problem
///
/// When orders exist in both base markets and joint markets (bundles),
/// marginal consistency requires:
/// ```text
/// P(base outcome) = Σ P(joint outcomes where base has that outcome)
/// ```
///
/// # Solution
///
/// Solves a small QP over prices only (~1000-6000 variables):
/// ```text
/// minimize ||p_raw - p||²
/// subject to: marginal consistency constraints
/// ```
pub trait PriceProjector: Send + Sync {
    /// Project prices to satisfy marginal consistency.
    ///
    /// # Arguments
    /// * `prices` - Raw prices from price discovery
    /// * `problem` - The problem (used to extract constraints from orders)
    fn project(
        &self,
