...
use sybil_l1_protocol::{Bytes32, EthAddress, L1ProtocolError};
...
use cursor::{BlockCheckpoint, CursorState, IntegrityIncident, load_cursor, save_cursor};
use events::{
...
use source::{AuthenticatedBlock, HttpEndpoint, RpcTrustMode, SourceIdentity, TrustedL1};

/// Default confirmation delay for explicitly unsafe local Anvil mode.
...
#[derive(Debug, Parser)]
struct Args {
    /// Comma-separated Ethereum JSON-RPC URLs. Public operation requires at
    /// least two independently operated providers in unanimous-finalized mode.
    #[arg(
        long,
        env = "SYBIL_L1_RPC_URLS",
        value_delimiter = ',',
        default_value = "http://127.0.0.1:8545"
    )]
    rpc_urls: Vec<String>,
    /// Comma-separated, non-secret operator-assigned identities corresponding
    /// one-for-one with `rpc_urls`. These identities are persisted in the
    /// deployment-bound cursor; never put URL credentials here.
    #[arg(
        long,
        env = "SYBIL_L1_RPC_IDS",
        value_delimiter = ',',
        default_value = "local-anvil"
    )]
    rpc_ids: Vec<String>,
    /// L1 trust boundary. The safe public mode requires unanimous independent
    /// providers and the standardized finalized tag. The unsafe mode is only
    /// for disposable local Anvil plumbing.
    #[arg(
        long,
        env = "SYBIL_L1_TRUST_MODE",
        value_enum,
        default_value = "unanimous-finalized"
    )]
    trust_mode: RpcTrustMode,
    /// Sybil API base URL.
    ...
    start_block: u64,
    /// Local-only confirmation depth used by unsafe-single-dev. The
    /// unanimous-finalized mode ignores this heuristic and uses the finalized
    /// tag instead.
    #[arg(long, env = "SYBIL_L1_CONFIRMATIONS", default_value_t = DEFAULT_CONFIRMATIONS)]
    confirmations: u64,
    /// Local-only minimum confirmation depth enforced in unsafe-single-dev.
    /// Explicit `0` is permitted only for disposable local development.
    #[arg(
        long,
...
enum IndexerError {
    ...
    #[error("invalid L1 provider configuration: {0}")]
    ...
    #[error(
        "deposit root mismatch for deposit {deposit_id} at L1 block {block_number}: \
         on-chain depositRootByCount={onchain}, \
    ...
    #[error(
        "cursor state at {path} uses L1 source mode={stored_mode} providers={stored_providers:?}, \
         but this run configured mode={configured_mode} providers={configured_providers:?}; \
         refusing to change the trust boundary for an acknowledged cursor"
    )]
    ...
    #[error(
        "L1 providers disagree during {context} at block {block_number}: provider \
         {expected_provider} reported {expected}, provider {observed_provider} reported \
         {observed}; refusing further bridge input"
    )]
    ...
    #[error(
        "authenticated L1 view is invalid during {context} at block {block_number} from \
         provider {provider}: {message}; refusing further bridge input"
    )]
    AuthenticatedViewInvalid {
        context: &'static str,
        block_number: u64,
        provider: String,
        message: String,
    ...
    #[error(
        "authenticated L1 tip regressed: tip={tip_block}, previously_authenticated_tip=\
         {previous_tip_block}; refusing further bridge input"
    )]
    FinalityRegression {
        tip_block: u64,
        previous_tip_block: u64,
    },
    #[error(
...
impl IndexerError {
    ...
    fn metric_kind(&self) -> &'static str {
        match self {
            ...
            Self::IntegrityIncidentLatched { .. } => "integrity_latched",
            Self::CanonicalBlockHashMismatch { .. } => "canonical_hash_mismatch",
            Self::ProviderDisagreement { .. } => "provider_disagreement",
            Self::AuthenticatedViewInvalid { .. } => "authenticated_view_invalid",
    ...
    fn is_latched_integrity(&self) -> bool {
        matches!(self, Self::IntegrityIncidentLatched { .. })
    }
}
...
fn check_provider_url_safety(mode: RpcTrustMode, urls: &[reqwest::Url]) -> Result<()> {
    if mode != RpcTrustMode::UnanimousFinalized {
        return Ok(());
    }
    let unique = urls
        .iter()
    ...
    if unique.len() != urls.len() {
        return Err(IndexerError::InvalidProviderConfig(
            "unanimous-finalized requires distinct RPC URLs; duplicate endpoints do not add an independent view"
                .to_string(),
        ));
    }
    Ok(())
...
fn warn_unsafe_single_provider(confirmations: u64) {
    tracing::warn!(
        confirmations,
        "DEV-ONLY unsafe-single-dev L1 trust mode has no authenticated public-finality claim"
    );
}
...
struct IntegrityLatchContext<'a> {
    next_from: u64,
    vault_hex: &'a str,
    chain_id: u64,
    source_identity: &'a SourceIdentity,
    checkpoint: Option<&'a BlockCheckpoint>,
    source_tip: Option<&'a BlockCheckpoint>,
}

fn persist_integrity_latch(
    path: &std::path::Path,
    cursor: IntegrityLatchContext<'_>,
    error: &IndexerError,
) -> Result<bool> {
    let (context, block_number, expected, observed) = match error {
        IndexerError::CanonicalBlockHashMismatch {
            context,
            block_number,
            expected,
            observed,
        } => (*context, *block_number, expected.clone(), observed.clone()),
        IndexerError::ProviderDisagreement {
            context,
            block_number,
            expected_provider,
            ...
            *context,
            *block_number,
            format!("provider={expected_provider} value={expected}"),
            format!("provider={observed_provider} value={observed}"),
        ),
        IndexerError::AuthenticatedViewInvalid {
            context,
            block_number,
            ...
            *context,
            *block_number,
            "valid authenticated view".to_string(),
        ...
        IndexerError::FinalityRegression {
            ...
            "authenticated finality regression",
            *previous_tip_block,
            format!("tip >= {previous_tip_block}"),
            format!("tip={tip_block}"),
        ),
        IndexerError::DepositRootMismatch {
            deposit_id,
            block_number,
            ...
            "deposit root reconciliation",
            *block_number,
            format!("deposit={deposit_id} block_state={onchain}"),
            format!("deposit={deposit_id} log_root={log}"),
        ),
        _ => return Ok(false),
    };
    let halted = CursorState::halted(
        ...
        cursor.vault_hex,
        cursor.chain_id,
        cursor.source_identity.clone(),
        cursor.checkpoint.cloned(),
        cursor.source_tip.cloned(),
        IntegrityIncident::new(context, block_number, expected, observed),
    );
...
trait L1Rpc {
    ...
    /// `depositRootByCount(count)` read via EIP-1898 `eth_call`, pinned to the
    /// deposit log's exact block hash with `requireCanonical=true`.
        ...
        outcome = &mut indexer => match outcome {
            ...
            Err(failure) => {
                metrics.record_fatal(
                    ...
                    failure.error.is_latched_integrity(),
                );
...
async fn run_indexer(args: &Args, metrics: &IndexerMetrics) -> std::result::Result<(), RunFailure> {
    metrics.configure_source(args.trust_mode.as_str(), args.rpc_ids.len());
    if args.trust_mode == RpcTrustMode::UnsafeSingleDev {
        warn_unsafe_single_provider(args.confirmations);
    ...
    if args.rpc_urls.len() != args.rpc_ids.len() {
        return Err(RunFailure::fatal(IndexerError::InvalidProviderConfig(
            format!(
                "rpc_urls has {} entries but rpc_ids has {}",
                args.rpc_urls.len(),
                args.rpc_ids.len()
            ),
        )));
    ...
    let rpc_urls = args
        ...
        .iter()
        .map(|url| {
            reqwest::Url::parse(url).map_err(|error| IndexerError::InvalidRpcUrl(error.to_string()))
        })
        .collect::<Result<Vec<_>>>()
        .map_err(RunFailure::fatal)?;
    check_provider_url_safety(args.trust_mode, &rpc_urls).map_err(RunFailure::fatal)?;
    let mut endpoints = Vec::with_capacity(rpc_urls.len());
    for (id, rpc_url) in args.rpc_ids.iter().zip(rpc_urls) {
        endpoints.push(HttpEndpoint::new(
            id.trim().to_string(),
            ProviderBuilder::new()
    ...
    let l1 = TrustedL1::new(endpoints, args.trust_mode).map_err(RunFailure::fatal)?;
    let source_identity = l1.identity().clone();
    tracing::info!(
        trust_mode = args.trust_mode.as_str(),
        provider_count = source_identity.provider_ids.len(),
        provider_ids = ?source_identity.provider_ids,
        "l1.indexer.trust_policy"
    );
    ...
    let persisted_cursor = load_cursor(
        &args.cursor_path,
        &vault_hex,
        args.chain_id,
        &source_identity,
    ...
    let mut source_tip = persisted_cursor
        .as_ref()
        .and_then(|state| state.source_tip.clone());
    match persisted_cursor.as_ref() {
        Some(state) => tracing::info!(
            ...
            authenticated_source_tip = state.source_tip.as_ref().map(|value| value.block_number),
        ...
        .await
        {
            Ok(poll) => {
                if let Some(progress) = poll.progress {
                    ...
                    source_tip = Some(progress.source_tip);
                }
            ...
            Err(error) => {
                match persist_integrity_latch(
                    &args.cursor_path,
                    IntegrityLatchContext {
                        next_from,
                        vault_hex: &vault_hex,
                        chain_id: args.chain_id,
                        source_identity: &source_identity,
                        checkpoint: checkpoint.as_ref(),
                        source_tip: source_tip.as_ref(),
                    },
                    &error,
                ) {
                    Ok(true) => {
                    ...
                    Err(save_error) => {
                        ...
                        tracing::error!(
                            ...
                            %save_error,
                            path = %args.cursor_path.display(),
                            "l1.indexer.integrity_latch_persist_failed"
                        );
...
async fn poll_once<L: L1Rpc, S: DepositSink>(
    ...
    persisted_source_tip: Option<&BlockCheckpoint>,
) -> Result<PollResult> {
    ...
    let authenticated_tip = l1.authenticated_tip(args.confirmations).await?;
    let previous_tip_block = persisted_source_tip
        .map(|tip| tip.block_number)
        .or_else(|| checkpoint.map(|checkpoint| checkpoint.block_number));
    if let Some(previous_tip_block) = previous_tip_block
        && authenticated_tip.number < previous_tip_block
    {
        return Err(IndexerError::FinalityRegression {
            tip_block: authenticated_tip.number,
            previous_tip_block,
        });
    }
    if let Some(previous_tip) = persisted_source_tip {
        let expected = previous_tip.block_hash(&args.cursor_path)?;
        require_canonical_block_hash(
            l1,
            previous_tip.block_number,
            expected,
            "persisted authenticated source tip",
        )
        .await?;
    }
    // In unanimous-finalized mode this is the lowest finalized height shared
    // by every configured provider. In explicitly unsafe local mode it is the
    // single endpoint's latest height minus the configured confirmation delay.
    let latest = authenticated_tip.number;
    let confirmed_tip = authenticated_tip.number;
    ...
    let range_tip_hash = if to == authenticated_tip.number {
        authenticated_tip.hash
    } else {
        l1.block_hash(to).await?
    };
    let (deposit_logs, withdrawal_logs) = l1.bridge_logs(vault_address, next_from, to).await?;
    let mut deposits = deposit_logs
        .into_iter()
        .map(indexed_deposit_from_log)
        .collect::<Result<Vec<_>>>()?;
    let mut withdrawal_events = withdrawal_logs
        .into_iter()
    ...
    for deposit in deposits {
        ...
        // Reconcile the log's cumulative root against the state committed by
        // the exact same authenticated block header. The production source
        // executes this EIP-1898 call with requireCanonical=true on every
        // provider and requires unanimous output before returning.
        let deposit_block = AuthenticatedBlock {
            number: deposit
                .log
                .block_number
        ...
        let onchain_root = l1
            .deposit_root_by_count(vault_address, deposit.event.deposit_id, deposit_block)
            .await?;
        if onchain_root != deposit.event.deposit_root {
            return Err(IndexerError::DepositRootMismatch {
                deposit_id: deposit.event.deposit_id,
                block_number: deposit_block.number,
    ...
    Ok(PollResult {
        progress: Some(ScanProgress {
            ...
            source_tip: BlockCheckpoint::new(authenticated_tip.number, authenticated_tip.hash),
        }),
        latest_block: latest,
        confirmed_tip_block: confirmed_tip,
    })
...
async fn submit_deposit<S: DepositSink>(
    ...
    // and SybilSettlement's independent vault check. In public mode the indexer
    // additionally requires unanimous independently operated providers for the
    // finalized header, EIP-234 block-hash-pinned logs, and EIP-1898 canonical
    // state call. This is an explicit at-least-one-honest-provider model, not an
    // embedded Ethereum light client, so the API route remains service-gated.
...
mod tests {
    ...
    impl L1Rpc for FakeL1 {
        async fn authenticated_tip(&self, confirmations: u64) -> Result<AuthenticatedBlock> {
            let number = self.latest.saturating_sub(confirmations);
            Ok(AuthenticatedBlock {
                ...
                hash: self
                    ...
                    .get(&number)
                    .copied()
                    .unwrap_or_else(|| test_block_hash(number)),
            })
        }
        ...
        async fn deposit_root_by_count(
            ...
            self.reconciled.lock().unwrap().push((count, block.number));
    ...
    fn test_args(confirmations: u64) -> Args {
        Args {
            ...
            rpc_ids: vec!["local-anvil".to_string()],
            trust_mode: RpcTrustMode::UnsafeSingleDev,
    ...
    #[test]
    fn finalized_provider_urls_must_be_distinct() {
        let duplicate = reqwest::Url::parse("https://rpc.example.test").unwrap();
        assert!(matches!(
            check_provider_url_safety(
        ...
        assert!(
            check_provider_url_safety(
                RpcTrustMode::UnanimousFinalized,
                &[
                    reqwest::Url::parse("https://rpc-a.example.test").unwrap(),
                    reqwest::Url::parse("https://rpc-b.example.test").unwrap(),
                ]
            )
            .is_ok()
        );
    ...
    #[test]
    fn unsafe_single_provider_mode_emits_dev_only_warning() {
        ...
        tracing::subscriber::with_default(subscriber, || warn_unsafe_single_provider(2));

        let logs = logs.contents();
        assert!(logs.contains("WARN"));
        assert!(logs.contains("DEV-ONLY unsafe-single-dev L1 trust mode"));
    }
    ...
    async fn credits_deposit_when_onchain_root_matches() {
        ...
        // Reconciliation is pinned to the deposit log's own authenticated block.
        assert_eq!(l1.reconciled.lock().unwrap().as_slice(), &[(1, 2)]);
    ...
    #[tokio::test]
    async fn authenticated_tip_regression_below_previously_observed_tip_is_fatal() {
        let vault = [0x10; 20];
        let l1 = FakeL1 {
            latest: 95,
            ..Default::default()
        ...
        let checkpoint = BlockCheckpoint::new(89, test_block_hash(89));
        let previous_source_tip = BlockCheckpoint::new(96, test_block_hash(96));
        ...
        assert!(matches!(
            ...
            IndexerError::FinalityRegression {
                tip_block: 95,
                previous_tip_block: 96
            }
        ));
        assert!(error.is_fatal());
        assert_eq!(l1.block_hash_queries.lock().unwrap().as_slice(), &[89]);
        assert!(l1.deposit_log_ranges.lock().unwrap().is_empty());
        assert!(sink.observed_heights.lock().unwrap().is_empty());
    }
    ...
    fn cursor_persistence_roundtrip() {
        ...
        assert_eq!(
            load_cursor(&path, &vault_hex, 31_337, &source).unwrap(),
            None
        );
    ...
    fn cursor_rejects_mismatched_vault_or_chain() {
        ...
        assert!(matches!(
            load_cursor(&path, &other_vault, 31_337, &source),
            Err(IndexerError::CursorConfigMismatch { .. })
        ));
        assert!(matches!(
            load_cursor(&path, &vault_hex, 1, &source),
            Err(IndexerError::CursorConfigMismatch { .. })
        ));
    ...
    #[test]
    fn cursor_rejects_changed_provider_trust_boundary() {
        let dir = std::env::temp_dir().join(format!(
            "syb86-cursor-provider-mismatch-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("cursor.json");
        ...
        let changed = SourceIdentity::new(
            RpcTrustMode::UnanimousFinalized,
            vec!["provider-a".to_string(), "provider-c".to_string()],
        );
        assert!(matches!(
            load_cursor(&path, &vault_hex, 31_337, &changed),
            Err(IndexerError::CursorSourceMismatch { .. })
        ));
    ...
    fn legacy_cursor_without_checkpoint_is_rejected() {
        ...
        assert!(matches!(
            load_cursor(&path, &vault_hex, 31_337, &test_source_identity()),
    ...
    #[tokio::test]
    async fn persisted_integrity_incident_is_fail_stop_latched_across_restart() {
        ...
        assert!(
            persist_integrity_latch(
                ...
                IntegrityLatchContext {
                    ...
                    checkpoint: Some(&BlockCheckpoint::new(8, expected)),
                    source_tip: Some(&BlockCheckpoint::new(8, expected)),
                },
                &error,
            )
            .unwrap()
        ...
        assert!(matches!(
            load_cursor(&path, &vault_hex, 31_337, &test_source_identity()),
            Err(IndexerError::IntegrityIncidentLatched {
                context,
                block_number: 8,
        ...
        let failure = run_indexer(&args, &metrics).await.unwrap_err();
        assert!(failure.fatal);
        assert_eq!(failure.error.metric_kind(), "integrity_latched");
        assert!(failure.error.is_latched_integrity());
        ...
        let rendered = metrics.render();
        assert!(rendered.contains("sybil_l1_indexer_integrity_latched 1"));
        assert!(
            rendered
                .contains("sybil_l1_indexer_fatal_failures_total{kind=\"integrity_latched\"} 1")
        );
    ...
    #[test]
    fn provider_disagreement_evidence_is_persisted_with_source_identity() {
        let dir =
            std::env::temp_dir().join(format!("syb86-provider-incident-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("cursor.json");
        let vault_hex = hex::encode([0x10; 20]);
        let source = SourceIdentity::new(
        ...
        let error = IndexerError::ProviderDisagreement {
            context: "unanimous finalized prefix",
            block_number: 96,
            expected_provider: "provider-a".to_string(),
            observed_provider: "provider-b".to_string(),
            expected: hex::encode([0x11; 32]),
            observed: hex::encode([0x22; 32]),
        };
        ...
        assert!(
            persist_integrity_latch(
                ...
                IntegrityLatchContext {
                    ...
                    checkpoint: Some(&BlockCheckpoint::new(89, test_block_hash(89))),
                    source_tip: Some(&BlockCheckpoint::new(96, test_block_hash(96))),
                },
                &error,
            )
            .unwrap()
        ...
        assert!(json.contains("\"trust_mode\": \"unanimous-finalized\""));
        assert!(json.contains("\"provider-a\""));
        assert!(json.contains("\"provider-b\""));
        assert!(json.contains("\"integrity_incident\""));
        assert!(json.contains("provider=provider-a"));
        assert!(json.contains("provider=provider-b"));
        assert!(matches!(
            load_cursor(&path, &vault_hex, 1, &source),
            Err(IndexerError::IntegrityIncidentLatched {
                block_number: 96,
