pub(crate) fn append_order(value: &mut Vec<u8>, order: &matching_engine::Order) {
    ...
    value.extend_from_slice(&order.limit_price.0.to_le_bytes());
    value.extend_from_slice(&order.max_fill.0.to_le_bytes());
    match &order.condition {
        None => value.push(0),
        Some(condition) => {
            value.push(1);
            value.extend_from_slice(&condition.market.0.to_le_bytes());
            value.extend_from_slice(&condition.threshold.0.to_le_bytes());
