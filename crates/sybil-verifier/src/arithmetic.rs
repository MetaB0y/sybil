//! Overflow-safe arithmetic helpers for verification.
//!
//! All price*qty computations go through i128 intermediates to avoid overflow.

use matching_engine::{Nanos, Qty};

/// Compute `price * qty` as i64, using i128 intermediate.
/// Returns `None` on overflow (value doesn't fit in i64).
pub fn checked_price_qty(price: Nanos, qty: Qty) -> Option<i64> {
    let result = (price as i128).checked_mul(qty as i128)?;
    i64::try_from(result).ok()
}

/// Compute `price * qty` as i64 (unchecked cast, matching sequencer behavior).
/// The sequencer does `(price as i128 * qty as i128) as i64`.
pub fn price_qty(price: Nanos, qty: Qty) -> i64 {
    (price as i128 * qty as i128) as i64
}

/// Compute welfare for a single fill.
///
/// Buyers: `(limit_price - fill_price) * fill_qty`
/// Sellers: `(fill_price - limit_price) * fill_qty`
pub fn welfare(limit_price: Nanos, fill_price: Nanos, fill_qty: Qty, is_seller: bool) -> i64 {
    if fill_qty == 0 {
        return 0;
    }
    let surplus_per_unit = if is_seller {
        fill_price as i64 - limit_price as i64
    } else {
        limit_price as i64 - fill_price as i64
    };
    surplus_per_unit * fill_qty as i64
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_price_qty_basic() {
        assert_eq!(price_qty(500_000_000, 100), 50_000_000_000);
    }

    #[test]
    fn test_checked_price_qty_overflow() {
        // u64::MAX * u64::MAX would overflow i64
        assert!(checked_price_qty(u64::MAX, u64::MAX).is_none());
    }

    #[test]
    fn test_welfare_buyer() {
        // Buyer: limit=0.60, fill=0.50, qty=100 => welfare = 0.10 * 100
        let w = welfare(600_000_000, 500_000_000, 100, false);
        assert_eq!(w, 10_000_000_000);
    }

    #[test]
    fn test_welfare_seller() {
        // Seller: limit=0.40, fill=0.50, qty=100 => welfare = 0.10 * 100
        let w = welfare(400_000_000, 500_000_000, 100, true);
        assert_eq!(w, 10_000_000_000);
    }

    #[test]
    fn test_welfare_zero_qty() {
        assert_eq!(welfare(600_000_000, 500_000_000, 0, false), 0);
    }
}
