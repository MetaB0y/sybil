...
11. **commonware 2026.5 migration — completed 2026-07-11**: the deps pass (`f693bdc7`) pinned all nine commonware crates back to 2026.4.0. The migration now uses the 2026.5 authenticated-storage config with explicit sequential strategies, mirrors backward proof bagging and inactive-peak commitments in the guest, encodes keyless commit floors, and refreshes the native/guest golden root. This crossed the qMDB proof boundary as expected and was handled as a soundness migration rather than a lockfile-only bump.
