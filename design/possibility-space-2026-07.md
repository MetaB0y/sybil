...
**Consensus impact** (does it move the guest commitment?), and **Horizon**.
...
consensus move). The payoff: **conditional markets** — "Fed cuts *if* CPI < 3%" —
...
(consensus-critical, `Order` generality + solver support). **Leverage: maximal.
Consensus: yes. Horizon: the flagship medium-term bet.**
...
Consensus: moderate (new outcome/resolution type). Horizon: medium.**
...
Consensus: shares #1's. Horizon: after #1.**
...
lifecycle. **Leverage: medium. Consensus: moderate. Horizon: medium-long.**
...
medium. Consensus: new settlement path. Horizon: opportunistic.**
...
and nearly free architecturally. Consensus: adds a decrypt step before clearing.
...
**Leverage: high (reuses the state commitment). Consensus: new small guest.
Horizon: medium.**
...
built, SYB-115). **Leverage: high (arena exists). Consensus: none. Horizon: a
...
digested field. **Leverage: high. Consensus: extends the keys_digest schema.
...
**Leverage: high (EG-specific). Consensus: touches settlement. Horizon: medium;
design-first.**
...
liquidity problem without abandoning the core. **Leverage: high. Consensus: the
seed is a participant, so mostly off-consensus. Horizon: needed before real
...
must trust today. **Leverage: high. Consensus: resolution path + `MarketStatus`
...
Consensus: DA commitment shape. Horizon: long.**
...
Consensus: none directly (it's an ops/liveness layer). Horizon: long.**
...
Consensus: state-root composition. Horizon: only when measured as needed.**
