...
| `ContainerRestartObserved` | A Compose service's Docker restart count increases or its started-at timestamp changes within 20 minutes | A replacement gets a new container whose restart count begins at zero, so the retained start timestamp covers both automatic restarts and whole-container recreation. The window spans one failed five-minute probe plus timer jitter; `SyntheticProbeMissing` covers longer telemetry gaps. |
