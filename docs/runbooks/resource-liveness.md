...
> **Executive summary:** these warnings catch unusually fast permanent-state
> growth, large full-state witnesses, and a post-commit DA writer that is
> falling behind. Block commit remains authoritative, so triage DA separately
> and preserve the store instead of treating every alert as a validity failure.

**Alerts:** `SequencerAccountStockGrowingFast`, `WitnessPayloadLarge`,
...
2. For account growth, inspect request volume and callers for
...
3. For a large witness, compare `sybil_state_accounts_total`,
...
4. For a DA backlog, compare p99 DA persistence duration with block cadence and
   ...
   artifacts.
5. Verify a recent manifest and payload through the service-gated DA endpoints
