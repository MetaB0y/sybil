...
This is deliberately finer grained than [[Exact Component Solver]]. The exact
router first joins every market touched by one market group, spanning order, or
shared MM budget and therefore needs no coordination. `DecomposedSolver`
instead attempts to split across shared budgets and pays for that extra
parallelism with an iterative coordination approximation.
