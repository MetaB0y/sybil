...
The Python SDK (`sybil_client`) is an async HTTP client that wraps the [[REST API]] for bot development. Built on `httpx`, it provides a `SybilClient` class with methods for every API endpoint plus convenience features: automatic [[Nanos and Integer Arithmetic|nanos]] conversion (pass prices as floats like 0.55 instead of 550,000,000), SSE block streaming as an async iterator, and typed response objects.

The core interaction pattern is straightforward. Create a client with the server URL, create an account with initial funds, submit orders using helper functions like `buy_yes(account_id, market_id, price, quantity)`, and stream blocks to see results. The SDK handles all the nanos conversion — you think in dollars and shares, it sends nanos over the wire. The `stream_blocks()` method wraps the [[SSE Block Stream]] into a Python async iterator: `async for block in client.stream_blocks()` gives you each block as a typed `Block` object with `clearing_prices`, `fills`, and account data.
...
## Key Properties
- Async `httpx`-based client: `SybilClient`
- Automatic nanos conversion: pass prices as floats (0.55 → 550,000,000)
- `stream_blocks()` — SSE wrapped as Python async iterator
...
- [[SSE Block Stream]] — real-time block push consumed by `stream_blocks()`
