...
flowchart TB
    ...
    subgraph exchange["Deterministic exchange"]
        ...
        WIT["BlockWitness v14<br/>private transition package"]
...
sequenceDiagram
    ...
    A->>V: Persist/export BlockWitness v14
...
| Genesis-bound signed trading actions | Captured orders/cancels/MM bundle lifecycle actions must not cross chain domains, replay, or split atomic intent | Witness v14 retains exact envelopes; verification replays active keys, committed trading nonces, and whole-bundle effects |
...
- WebAuthn/P256 admission, atomic authorization WAL rows, committed per-account trading nonces, and shared native/guest verification of key operations plus ordinary order/cancel and atomic MM-bundle intent.
- Witness v14, committed last clearing prices and group creation identities, deposit quarantine, exact-keyspace proofs, and guest/native commitment agreement. Deployment repinning is coordinated with the epoch guest.
