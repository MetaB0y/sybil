...
> Sybil minimizes trust in transition correctness. Witness v14 and shared
...
flowchart TB
    subgraph crypto["🟢 Implemented controls"]
        ...
        TRANS["State transition<br/>witness v14 · settlement · sidecar · exact qMDB proofs"]
...
|---|---|---|
| Forge a key registration/revocation | 🟢 | Witness v14 binds `genesis_hash`, the active key universe, state digests, key operation, and RawP256/WebAuthn envelope; shared native/guest verification replays authorization. |
| Forge an ordinary order/cancellation | 🟢 | Witness v14 retains the exact action envelope in actor order. Verification checks the active scheme-matching key, genesis/action binding, RawP256/WebAuthn policy and signature, committed trading nonce, and corresponding order/cancel effect. |
| Split, relabel, over-budget, or ignore a signed MM lifecycle action | 🟢 | Submit/replace/cancel signatures bind bundle identity and exact revision; successor signatures bind ordered orders, payoff-derived sides, one budget, exact next-block expiry, and nonce. Admission and witness verification require exact constraint coverage and an all-accepted/all-rejected successor, or no constraint after cancellation. The operator may censor a lifecycle message but cannot prove a different one. |
