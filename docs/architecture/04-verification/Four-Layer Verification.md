...
**Layer 1: Match Verification** checks that the solver's output is economically valid. Per-fill checks confirm that each filled order exists, has positive quantity, doesn't exceed the order's maximum, and respects the order's limit. System-wide checks enforce the Uniform Clearing Price (UCP), price complementarity (YES + NO = $1 for binary markets), [[MM Budget Constraint|MM budget]] compliance, and welfare consistency. The verifier recomputes signed complete-set cost from fill cash flow (positive for creation, negative for burning), requires reported welfare to match, and rejects negative total welfare. A market group's distance from a $1 YES-price sum is reported as a quality statistic only: with finite liquidity, absence of the arbitrage flow needed to close that distance is not an invalid state transition.
...
graph TB
    ...
    WITNESS --> L1["Layer 1: Match Verification<br/>positive fill quantities · fill limits · UCP<br/>binary complementarity · MM budgets · welfare"]
...
- Validity is mode-independent: diagnostics add statistics and never change pass/fail
- Zero-quantity fill records are invalid in both native and guest verification
- The same verification logic executes in the [[ZK Integration Path|OpenVM guest]]
