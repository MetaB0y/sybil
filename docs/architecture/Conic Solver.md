...
This is Theorem 5 from the theoretical work (see `paper.typ` in `~/github/prediction-markets-are-fisher-markets/`; pointer `design/math-papers.md`). In benchmarks, QuasiFisher achieves welfare within ~0.5% of the [[LP Solver]], nearly matching it in quality, while being theoretically cleaner about budget handling. The welfare gap stays below 0.7% across all budget scaling levels (from 0.1x to 1.5x the default budget), much tighter than the quadratic bound from Proposition 5 would predict. Known issue: at budget scales >= 2x, the cash variable grows large and Clarabel's convergence degrades.
