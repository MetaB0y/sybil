...
### Phase 3: NegriskSolver (Arbitrage)

When prices for mutually exclusive outcomes don't sum to exactly \$1, there's an arbitrage opportunity. Instead of artificially adjusting prices (which destroys welfare), the NegriskSolver creates **synthetic arbitrage orders** that influence prices through market forces.
...
In the sequential pipeline, arb orders participate in the next iteration's LocalSolver clearing to push prices toward sum = \$1. The arb fills themselves are filtered out of the final result — they serve as price pressure, not as output.
...
**Output**: Synthetic arb orders that drive prices toward consistency.
...
<Info>
The dual solver replaces the NegriskSolver's role — instead of creating synthetic arb orders, it uses bid shading (adjusting limit prices via lambda) to drive prices toward consistency. Both approaches achieve the same goal; dual decomposition is more principled while the sequential pipeline with NegriskSolver can be faster to converge in practice.
</Info>
